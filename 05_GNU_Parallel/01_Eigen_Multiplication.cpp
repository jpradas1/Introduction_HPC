            ...
            double & mean_ctime, double & sigma_ctime)
{
    ...
    // measure total times and its squares
    for (int rep = 0; rep < reps; ++rep){
        srand(rep+1); //change seed for each repettion
    ...
    // average of both wall and cpu time
    mean_wtime = wsum/reps;
    mean_ctime = csum/reps;

    // standard deviation for both times
...
void multiply(const int size, double & wtime, double & ctime)
{
    ...
    auto C{A*B}; // MULTIPLY
    double tmp = C(0,0); // use the matrix to force eigen compute it
